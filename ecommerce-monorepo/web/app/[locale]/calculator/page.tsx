...
export default function CalculatorPage() {
  ...
  return (
    <SharedLayout 
      ...
      breadcrumbs={[
        { name: t('breadcrumb'), href: '/calculator' }
      ]}
            ...
            <div className="col-span-1 lg:col-span-1 bg-white rounded-2xl shadow-brand p-6 border border-gray-100 h-fit">
              ...
              <form onSubmit={handleCalculate} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-600 mb-1">{t('form.origin')}</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-600 mb-1">{t('form.destination')}</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-600 mb-1">{t('form.weight')}</label>
                  <input
                    type="number"
                    step="0.01"
                    placeholder="e.g. 150"
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-600 mb-1">{t('form.dimensions')}</label>
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-600 mb-1">{t('form.freightSpeed')}</label>
                  <select
                    value={serviceType}
                  ...
                  >
                    <option value="air">{t('freightSpeed.air')}</option>
                    <option value="sea">{t('freightSpeed.sea')}</option>
                    <option value="express">{t('freightSpeed.express')}</option>
                  </select>
                </div>
                ...
                >
                  {loading ? t('estimating') : t('getEstimation')}
            ...
            <div className="col-span-1 lg:col-span-2 space-y-6">
              {!estimate && !loading ? (
                <div className="bg-white rounded-2xl shadow-brand p-8 border border-gray-100 text-center flex flex-col justify-center items-center h-full min-h-[350px]">
                  ...
                  <h3 className="text-lg font-bold text-gray-800">{t('results.noEstimation')}</h3>
                ...
                <div className="bg-white rounded-2xl shadow-brand p-6 border border-gray-100 space-y-6">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 border-b border-gray-100 pb-4">{t('results.quotationPreview')}</h3>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50 rounded-xl p-6 border border-gray-100">
                    <div>
                      ...
                      <LocaleLink href={`/quotes?service=${serviceType}&weight=${weight}&origin=${origin}&destination=${destination}`}
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-gray-700 mb-3 uppercase tracking-wider">{t('results.calcDetails')}</h4>
                    <div className="border border-gray-100 rounded-xl divide-y divide-gray-100 text-sm">
                      <div className="flex justify-between p-3.5">
                        <span className="text-gray-500">{t('results.serviceRate')}</span>
                        <strong className="text-gray-800">${estimate?.breakdown.baseRate.toFixed(2)} / kg</strong>
                      </div>
                      <div className="flex justify-between p-3.5">
                        <span className="text-gray-500">{t('results.totalWeight')}</span>
                        <strong className="text-gray-800">{estimate?.breakdown.weightKg} kg</strong>
                      </div>
                      <div className="flex justify-between p-3.5">
                        <span className="text-gray-500">{t('results.speedFactor')}</span>
                        <strong className="text-gray-800">x{estimate?.breakdown.multiplier}</strong>
                      </div>
                      <div className="flex justify-between p-3.5">
                        <span className="text-gray-500">{t('results.routeFactor', { destination })}</span>
                      ...
                      {insurance && (
                        <div className="flex justify-between p-3.5 bg-emerald-50/50">
                          <span className="text-emerald-700 font-medium">{t('results.insurance')}</span>
